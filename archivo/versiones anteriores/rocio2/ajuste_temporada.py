...
def fun(parametros):
    ...
    for t in range(1,dias):
        ...
        solucion     =  rk4py2(modelo,t,h,v,args=(EV,H_t,beta_day,Kmax))
        v = solucion
...
def objetive(trial):
    x1 = trial.suggest_float("x1",1,2.8) #beta day
    x2 = trial.suggest_float("x2",1,2.8)
    x3 = trial.suggest_float("x3",1,2.8)
    x4 = trial.suggest_float("x4",1,2.8)
    x5 = trial.suggest_float("x5",1,2.8)
    x6 = trial.suggest_float("x6",1,2.8)
    x7 = trial.suggest_float("x7",1,2.8)
    x8 = trial.suggest_float("x8",1,2.8)

    y1 = trial.suggest_float("y1", 1, 3000) #kmax
    y2 = trial.suggest_float("y2", 1, 3000)
    y3 = trial.suggest_float("y3", 1, 3000)
    y4 = trial.suggest_float("y4", 1, 3000)
    y5 = trial.suggest_float("y5", 1, 3000)
    y6 = trial.suggest_float("y6", 1, 3000)
    y7 = trial.suggest_float("y7", 1, 3000)
    y8 = trial.suggest_float("y8", 1, 3000)

    parametros = [x1,x2,x3,x4,x5,x6,x7,x8,y1,y2,y3,y4,y5,y6,y7,y8]
...
study.optimize(objetive, n_trials = 250)
...
print("FOUND b9:{} ,b10:{}, b11:{}, b12:{},b13:{},b14:{},b15:{},b16:{},k9:{},k10:{},k11:{},k12:{},k13:{},k14:{},k15:{},k16:{}".
      format(found_x1,found_x2,found_x3,found_x4,found_x5,found_x6,found_x7,found_x8,found_y1,found_y2,found_y3,found_y4,found_y5,found_y6,found_y7,found_y8,))
