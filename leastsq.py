...
data=readinputdata("datos_clinicos_2010_2016.txt")
...
for i in range(DAYS):
    ref=ref+1
    if(ref>6):
        importados[i]=np.round(random.uniform(0,26),decimals=2)
        ref=volver
    else:
        importados[i]=0.
...
def model(z,t,b_day,b_pica):
    ...
    dVidt = EV - muV*Vi
    dHsdt = -b_theta_bh * (Hs/H)*Vi - sigmaH*He
...
def fun(parametros,meses):
    ...
    """
    c_sim_2008_2016=c_sim_week[105:489]
    ordenado=c_sim_2008_2016.reshape((96,4))
    for i in range(0,96):
        for j in range(0,4):
            ref = ref + ordenado[i][j]
            c_sim_mes[i]=ref
            ref=volver
    """
...
def diferencia(parametros,y,meses):
    return y - fun(parametros,meses)

#a=np.size(c_sim_mes)
parametros=np.array([1 , 0.1],dtype=float)
meses=np.linspace(0,84,84)
delta_beta_day=0.01
delta_beta_rite=0.01
"""
error = diferencia(parametros, casos_clinicos, meses)
error_2= pow(error,2)
"""
...
"""
while (parametros[0]<2.87):
    error_2= sum(pow( diferencia(parametros,casos_clinicos,meses),2 ))
    with open("error.txt","a") as file:
        #file.write("\n")
        file.write( str(error_2) +'\t'+ str(parametros) + '\n')
    parametros[0] = parametros[0] + delta_beta_day
    if (error_2 <= 0.1):
        break
"""

while(parametros[1]<0.41):
    #parametros[1]=parametros[1] + delta_beta_day
    for i in np.arange(1,2.86,delta_beta_day):
        parametros[0]=i
        error_2= (np.square(diferencia(parametros,casos_clinicos,meses))).mean() ## Error Cuadrático medio ##
        with open("error.txt","a") as file:
            #file.write("\n")
            file.write( str(error_2) +'\t'+ str(parametros) + '\n')
        #parametros[0] = parametros[0] + delta_beta_day
        if (error_2 <= 0.1):
            print(parametros)
    parametros[1]=parametros[1] + delta_beta_rite


#x,flag=leastsq(diferencia,parametros,args=(casos_clinicos,meses))
#print(sum(error_2))
#beta_day=result[0][0]
#bite_rate=result[0][1]

#print('beta_day=' + str(beta_day) + ',bite_rate=' + str(bite_rate))
