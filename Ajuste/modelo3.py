...
inicio = [1,'ene',2007]
final = [31,'dic',2017]
beta_day = 2.05   #1.56 // 1.95 // 1.33
bite_rate = 0.21  #0.32 // 0.25 // 0.44

tiempo1=np.arange('2007-01-02','2017-12-31',dtype='datetime64[D]')
###################################################################
...
#beta_day            = 1.33 #// or 0.25 or 1.19 or 0.47 or 0.5// 2.39
#bite_rate           = 0.44#//0.3//0.342//0.31//0.39//0.3875//0.39//0.3875//0.3875//16.15 // 2.59
...
#print(semanas)
G_T_week = np.zeros(semanas)
G_TV_week = np.zeros(semanas)
contar = 0 
week = 0
CI=2
#print(dias)
...
WEEKS = np.linspace(0,semanas,semanas)
casos_mes = orden(G_T_week)
mm = np.size(casos_mes)
meses = np.linspace(0,mm,mm)
tiempo2=np.arange('2009-01-01','2017-01-31',dtype='datetime64[M]')
#RR=moving_average(parametro, 7)
#np.savetxt("RR.txt",RR,fmt='%1.4e')
#paso=np.linspace(0,176,176)
#print(paso_d[0:3],control[0:3])
#print(paso_d)
#print(G_T[8:15],sum(G_T[8:15]),G_T_week[1])


plt.figure(figsize=(10,6))
#plt.plot(paso_d,parametro,label='R_0')
plt.plot(tiempo2,casos_clinicos, 'r',lw=2, label='casos reportados')
plt.plot(tiempo2,casos_mes, '--g',lw=2 ,label='simulacion')
plt.title(periodo)
plt.grid()
plt.legend()
plt.show()
